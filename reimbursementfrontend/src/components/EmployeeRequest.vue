    ...
    <div class="p-8">
        ...
        <div v-if="department == 'HR' || department == 'Finance'" class="overflow-x-auto">
            <table class="min-w-full border-collapse bg-white shadow-md rounded-lg">
                ...
                <tbody>
                    <tr v-for="(req, index) in request" :key="req.id" class="hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 border-b flex gap-2 text-sm">
                            ...
                            <template v-if="department == 'HR'">
                                ...
                                <button v-if="!(department == 'HR' && req?.request?.isApprovedByHr)"
                            ...
                            <template v-else-if="department == 'Finance'">
                                <button
                                    ...
                                    :class="req?.statusName === 'Passed'
                                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                                        : 'bg-green-500 hover:bg-green-600 text-white'" class="px-3 py-1 rounded"
        ...
        <div v-if="department != 'HR' && department != 'Finance'" class="overflow-x-auto">
            <table class="min-w-full border border-gray-300 text-left">
                ...
                <tbody>
                    <tr v-for="(req, index) in request" :key="req.id" class="hover:bg-gray-50 h-6">
                        ...
                        <td class="py-2 px-4 border-b flex gap-2 justify-between">
                            ...
                            <button
                                v-if="(!req?.isApprovedByManager && (req?.statusName === 'Pending')) || (req?.statusName === 'Rejected')"
                                :class="{
                                ...
                                }" class="px-3 py-1 rounded w-full " @click="openModal('reject', req?.id)">
        ...
        <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div class="bg-white p-6 rounded shadow-lg max-w-lg w-full overflow-y-auto"
                ...
                <button @click="closeModal" class="mt-4 px-4 py-2 bg-gray-700 text-white rounded hover:bg-gray-800">
                    Close
                </button>
            </div>
        </div>
...
import { getRequestbyManagerId } from '@/scripts/Request';
...
export default {
    ...
    methods: {
        async GetRequestManager() {
            try {
                ...
                this.request.sort((a, b) => {
                    // Define the priority for `status` and `isApprovedByManager`
                    ...
                    return priorityB - priorityA; // Sort by ascending priority
                });
        ...
        //     const managerId = jwtDecode(sessionStorage.getItem('token'))['Id']
        //     var result = await approveRequest(+id, +managerId, "Approved By Manager")
        //     console.log(result);
